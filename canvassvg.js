...
var CanvasSVG = function( selector , context ) {
	...
	this.sync = function() {
		...
		this.context.lineJoin = this.style.lineJoin = this.lineJoin;
		this.context.miterLimit = this.style.miterLimit = this.miterLimit;
		this.context.fillStyle = this.style.fillStyle = this.fillStyle;
		this.context.shadowOffsetX = this.style.shadowOffsetX = this.shadowOffsetX;
		this.context.shadowOffsetY = this.style.shadowOffsetY = this.shadowOffsetY;
		this.context.shadowBlur = this.style.shadowBlur = this.shadowBlur;
		this.context.shadowColor = this.style.shadowColor = this.shadowColor;
		this.context.font = this.style.font = this.font;
		this.context.textAlign = this.style.textAlign = this.textAlign;
...
CanvasSVG.prototype = {
	...
	stroke : function() {
		...
		this.svg.subpath.style.lineWidth = this.lineWidth;
		this.svg.subpath.style.lineCap = this.lineCap;
		this.svg.subpath.style.lineJoin = this.lineJoin;
	...
	toDataURL : function( type , args ) {
		...
		if( type == "image/svg+xml" ) {
			...
			for( var i=0; i < paths.length; i++ ) {
				var obj = paths[i];
				if( obj.type == "path" ) {
					xml += "<path d=\""+ obj.path +"\" "+convertStyle( obj.style )+" " + convertTransforms( obj.transforms ) + " />";
...
function convertStyle( params ) {
	...
	style+='"';
	if( count > 0 ) { return style; } else { return ""; }
}

function convertTransforms( params ) {
	var transforms = "";
	if( params != undefined && params.translate != undefined ) { transforms +="translate("+params.translate.x+","+params.translate.y+") "; }
	if( params != undefined && params.scale != undefined ) { transforms +="scale("+params.scale.x+","+params.scale.y+") "; }
	if( params != undefined && params.rotate != undefined ) { transforms +="rotate("+params.rotate.angle+","+params.rotate.cp1+","+params.rotate.cp2+") "; }
	return transforms;
}
