...
var SVGCanvas = (function() { 
	...
	SVGCanvas.fn = SVGCanvas.prototype = {
		...
		scale : function( x , y ) {
			...
			var pathtoadjust = null;
			
			if( currentPath.points.length > 0 ) {
				pathtoadjust = currentPath;
			} else {
				pathtoadjust = elements[elements.length-1];
			}
			
			for( var i=0; i < pathtoadjust.points.length; i++) {
				var path = pathtoadjust.points[i];
				path.x = path.x*x;
				path.y = path.y*y;
			}
		...
		rotate : function( angle ) {
			...
			xOrigin = pathtoadjust.points[0].x;
			yOrigin = pathtoadjust.points[0].y;
			
			if( pathtoadjust.points.length > 1 ) {
				for( var i=1; i < pathtoadjust.points.length; i++) {
					var path = pathtoadjust.points[i];
					path.x = ((path.x - xOrigin) * Math.cos(angle)) - ((yOrigin - path.y) * Math.sin(angle));
					path.y = ((yOrigin - path.y) * Math.cos(angle)) - ((path.x - xOrigin) * Math.sin(angle));
				}
			}
